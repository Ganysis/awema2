...
export const galleryPortfolio: Block = {
...
export function renderGalleryPortfolio(props: any): any {
  const {
    layout = 'grid',
    title,
    subtitle,
  ...
  const categories = [...new Set(images.map((img: any) => img.category))].filter(Boolean);

  const renderFilters = () => {
    if (!showFilter || categories.length === 0) return '';
  ...
  const renderImage = (image: any, index: number) => {
    ...
    switch(layout) {
      ...
      case 'grid-hover':
        return `
          <div class="gallery-item ${categoryClass} ${ratioClasses[imageRatio as keyof typeof ratioClasses]} overflow-hidden rounded-lg ${layout === 'grid-hover' ? 'group cursor-pointer' : ''}" data-aos="fade-up" data-aos-delay="${index * 50}">
            <img 
              src="${image.url}" 
              alt="${image.title}" 
      ...
      case 'cards':
        return `
          <div class="gallery-item ${categoryClass} bg-white rounded-lg shadow-lg overflow-hidden" data-aos="fade-up" data-aos-delay="${index * 50}">
            <div class="${ratioClasses[imageRatio as keyof typeof ratioClasses]} overflow-hidden">
              <img 
                src="${image.url}" 
                alt="${image.title}" 
      ...
      case 'carousel':
        return `
          <div class="swiper-slide">
            <div class="${ratioClasses[imageRatio as keyof typeof ratioClasses] || 'aspect-video'} overflow-hidden rounded-lg">
              <img 
                src="${image.url}" 
                alt="${image.title}" 
                class="w-full h-full object-cover"
      ...
      case 'before-after':
        if (!image.beforeUrl) return '';
        return `
          <div class="gallery-item ${categoryClass} before-after-container ${ratioClasses[imageRatio as keyof typeof ratioClasses]} rounded-lg overflow-hidden" data-aos="fade-up" data-aos-delay="${index * 50}">
  ...
  const renderGallery = () => {
    switch(layout) {
        ...
        return `
          <div class="swiper gallery-carousel">
            <div class="swiper-wrapper">
              ${images.map((img: any, idx: number) => renderImage(img, idx)).join('')}
            </div>
        ...
        return `
          <div class="tabs-container">
            ...
            <div class="tabs-content">
              <div class="tab-pane active grid grid-cols-1 ${columnClasses[columns as keyof typeof columnClasses]} ${spacingClasses[spacing as keyof typeof spacingClasses]}" id="all">
                ${images.map((img: any, idx: number) => renderImage(img, idx)).join('')}
              </div>
              ${categories.map((cat: any, idx: number) => `
                <div class="tab-pane hidden grid grid-cols-1 ${columnClasses[columns as keyof typeof columnClasses]} ${spacingClasses[spacing as keyof typeof spacingClasses]}" id="tab-${idx}">
                  ${images.filter((img: any) => img.category === cat).map((img: any, i: number) => renderImage(img, i)).join('')}
                </div>
              `).join('')}
            </div>
          </div>
      ...
      default:
        return `
          <div class="grid grid-cols-1 ${columnClasses[columns as keyof typeof columnClasses]} ${spacingClasses[spacing as keyof typeof spacingClasses]}">
            ${images.map((img: any, idx: number) => renderImage(img, idx)).join('')}
          </div>
        `;
    }
  ...
  return {
    html: `
      <section class="${backgroundClasses[backgroundColor as keyof typeof backgroundClasses]} py-16 md:py-24 gallery-section gallery-${layout}">
