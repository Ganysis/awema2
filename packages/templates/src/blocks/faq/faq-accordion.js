...
export function renderFaqAccordion(props) {
    const { layout = 'accordion-classic', title, subtitle, questions = [], expandFirst = true, showIcon = true, iconStyle = 'plus-minus', colorScheme = 'primary', backgroundColor = 'white', showCTA = true, ctaText, ctaButtonText, ctaButtonHref } = props;
    ...
    const renderIcon = (isOpen = false) => {
      ...
      <svg class="w-5 h-5 ${colors.icon} transition-transform duration-200 ${isOpen ? '' : 'rotate-0'}" 
        ...
        ${icons[iconStyle] || icons['plus-minus']}
      </svg>
    `;
    };
        ...
        switch (layout) {
            case 'tabs-categories':
                if (categories.length === 0) {
                    return `<div class="space-y-4">${questions.map((q, i) => renderQuestion(q, i)).join('')}</div>`;
                }
                return `
          <div class="tabs-faq">
            ...
            <div class="tabs-content">
              ...
              ${categories.map((cat) => `
                <div class="tab-pane hidden" id="${cat.toLowerCase().replace(/\s+/g, '-')}">
                  <div class="space-y-4">
                    ${questions.filter((q) => q.category === cat).map((q, i) => renderQuestion(q, i)).join('')}
                  </div>
                </div>
              `).join('')}
            </div>
    ...
    return {
        html: `
      <section class="${backgroundClasses[backgroundColor]} py-16 md:py-24 faq-section faq-${layout}">
