...
// Render function
export function renderTextImageBlock(props) {
    const { layout = 'image-right', style = 'clean', imageUrl, imagePosition = 'center', overline, title, titleSize = 'large', content, bulletPoints = [], primaryButtonText, primaryButtonHref, secondaryButtonText, secondaryButtonHref, backgroundColor = 'white', spacing = 'normal', animation = 'fade-up', imageEffects = [] } = props;
    // Style classes
    ...
    // Image effects
    const imageClasses = imageEffects.map((effect) => {
        switch (effect) {
    ...
    const renderTextContent = () => {
      ...
      <h2 class="${titleSizeClasses[titleSize]} font-bold mb-6">${title}</h2>
    ...
    return {
        html: `
      <section class="${backgroundClasses[backgroundColor]} ${spacingClasses[spacing]} ${styleVariants[style]} text-image-block text-image-${layout}">
