...
export const featuresMultiStyle = {
...
export function renderFeaturesMultiStyle(props) {
    const { layout = 'grid-icons', columns = '3', sectionTitle, sectionSubtitle, features = [], showNumbers = false, iconStyle = 'solid', backgroundColor = 'white', animation = 'fade-up' } = props;
    ...
    const renderIcon = (icon, color, index) => {
    ...
    return {
        html: `
      <section class="${backgroundStyles[backgroundColor]} py-16 md:py-24 features-section features-${layout}">
        <div class="container mx-auto px-4">
          ...
          <div class="grid grid-cols-1 ${gridCols[columns]} gap-8">
            ${features.map((feature, index) => renderFeature(feature, index)).join('')}
          </div>
        </div>
      </section>
    `,
