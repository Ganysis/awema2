...
export interface TypographyOptions {
  style?: 'modern' | 'classic' | 'playful' | 'elegant' | 'technical' | 'minimal';
  scale?: 'compact' | 'normal' | 'spacious';
  contrast?: 'low' | 'medium' | 'high';
}

export class TypographyGenerator {
  private static fontCombinations = {
    modern: [
   ...
   */
  static generateSystem(options: TypographyOptions = {}): TypographySystem {
    const {
      style = 'modern',
      scale = 'normal',
      contrast = 'medium'
   ...
   */
  private static generateFontSizes(baseSize: number, ratio: number): TypographyScale {
    const calculate = (steps: number): string => {
   ...
   */
  private static generateFontWeights(contrast: string): TypographySystem['fontWeight'] {
    switch (contrast) {
      case 'low':
        return {
          light: 300,
   ...
   */
  private static generateLineHeights(scale: string): TypographySystem['lineHeight'] {
   ...
   */
  private static generateLetterSpacing(style: string): TypographySystem['letterSpacing'] {
   ...
   */
  static generateCSSVariables(system: TypographySystem): string {
    return `
      :root {
        /* Font Families */
   ...
   */
  static getGoogleFontsImport(system: TypographySystem): string {
    const fonts = new Set([
   ...
   */
  static getIndustryPresets(): { [key: string]: TypographyOptions } {
    return {
