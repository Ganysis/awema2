...
export class ColorGenerator {
   ...
   */
  static generateScheme(options: ColorPaletteOptions = {}): ColorScheme {
    const {
   ...
   */
  private static generateHues(baseHue: number, scheme: string): {
    primary: number;
    secondary: number;
    accent: number;
  } {
   ...
   */
  static generateCSSVariables(scheme: ColorScheme): string {
    return `
      :root {
   ...
   */
  static getProfessionalSchemes(): { [key: string]: ColorScheme } {
    return {
      'tech-blue': {
  ...
  private static toRgba(_color: string, alpha: number): string {
 ...
 */
export function generateTradeColorSchemes(trade: string): ColorScheme[] {
  const tradeHues: { [key: string]: number[] } = {
