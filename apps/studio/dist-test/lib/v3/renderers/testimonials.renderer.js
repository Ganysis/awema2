...
exports.TestimonialsRendererV3 = void 0;
const testimonials_1 = require("../schemas/blocks/testimonials");
const logger_1 = require("../core/logger");
class TestimonialsRendererV3 {
    constructor() {
        this.type = 'testimonials-ultra-modern';
        this.version = '3.0.0';
        logger_1.logger.info('TestimonialsRendererV3', 'constructor', '🚀 Initialisation du renderer Testimonials V3');
    }
    validate(data) {
        logger_1.logger.debug('TestimonialsRendererV3', 'validate', 'Validation des données', {
            hasData: !!data,
            dataType: typeof data
        });
        const result = testimonials_1.testimonialsDataSchema.safeParse(data);
        if (!result.success) {
            logger_1.logger.warn('TestimonialsRendererV3', 'validate', 'Validation échouée', {
                errors: result.error.errors
            });
        }
        else {
            logger_1.logger.info('TestimonialsRendererV3', 'validate', '✅ Validation réussie', {
    ...
    getDefaultData() {
        logger_1.logger.debug('TestimonialsRendererV3', 'getDefaultData', 'Retour des données par défaut');
        return testimonials_1.testimonialsDefaults;
    }
    getDefaultCSS() {
        logger_1.logger.debug('TestimonialsRendererV3', 'getDefaultCSS', 'Génération CSS par défaut');
        return `
...
.testimonials--padding-xl { padding: 6rem 0; }`;
    ...
    getRequiredAssets() {
        logger_1.logger.debug('TestimonialsRendererV3', 'getRequiredAssets', 'Aucun asset requis');
        return [];
    }
    renderPreview(data) {
        logger_1.logger.debug('TestimonialsRendererV3', 'renderPreview', 'Génération preview', {
...
<div class="testimonials-preview">
  ...
  <p>${data.testimonials.length} témoignages - ${data.variant}</p>
    ...
    render(data, context) {
        ...
        logger_1.logger.info('TestimonialsRendererV3', 'render', '🎨 Début du rendu Testimonials', {
            variant: data.variant,
            testimonialsCount: data.testimonials.length,
        ...
        try {
            // Générer le HTML
            logger_1.logger.debug('TestimonialsRendererV3', 'render', 'Génération HTML');
            const html = this.generateHTML(data);
            // Générer le CSS spécifique
            logger_1.logger.debug('TestimonialsRendererV3', 'render', 'Génération CSS');
            const css = this.generateCSS(data);
            // Générer le JS si nécessaire
            logger_1.logger.debug('TestimonialsRendererV3', 'render', 'Génération JS');
            const js = this.generateJS(data);
            // Calculer les performances
            const renderTime = performance.now() - startTime;
            logger_1.logger.info('TestimonialsRendererV3', 'render', '✅ Rendu Testimonials terminé', {
            ...
            logger_1.logger.error('TestimonialsRendererV3', 'render', '❌ Erreur lors du rendu', error);
            return {
    ...
    generateHTML(data) {
        logger_1.logger.debug('TestimonialsRendererV3', 'generateHTML', 'Génération structure HTML', {
            variant: data.variant,
            layout: data.layout
        });
        const classes = [
  ...
  <div class="testimonials__container">
    ...
    renderHeader(data) {
        if (!data.title && !data.subtitle && !data.description)
            return '';
        logger_1.logger.debug('TestimonialsRendererV3', 'renderHeader', 'Génération header');
        return `
<div class="testimonials__header">
  ...
  ${data.description ? `<p class="testimonials__description">${this.escape(data.description)}</p>` : ''}
    ...
    renderStats(stats) {
        logger_1.logger.debug('TestimonialsRendererV3', 'renderStats', 'Génération stats', {
...
<div class="testimonials__stats">
  ...
  `).join('')}
    ...
    renderFilters(data) {
        logger_1.logger.debug('TestimonialsRendererV3', 'renderFilters', 'Génération filtres');
        // TODO: Implémenter les filtres
        return '';
    }
    renderTestimonials(data) {
        logger_1.logger.debug('TestimonialsRendererV3', 'renderTestimonials', 'Génération témoignages', {
    ...
    renderGridTestimonials(data) {
        const gridClasses = `testimonials__grid testimonials__grid--${data.layout.columns}`;
        return `
<div class="${gridClasses}">
  ${data.testimonials.map((testimonial, index) => this.renderTestimonial(testimonial, data, index)).join('')}
</div>`;
    }
    renderCarouselTestimonials(data) {
        return `
<div class="testimonials__carousel">
  ...
  ` : ''}
    ...
    renderMasonryTestimonials(data) {
        return `
<div class="testimonials__grid testimonials__masonry">
  ${data.testimonials.map((testimonial, index) => this.renderTestimonial(testimonial, data, index)).join('')}
</div>`;
    }
    renderListTestimonials(data) {
        return `
<div class="testimonials__grid testimonials__list">
  ${data.testimonials.map((testimonial, index) => this.renderTestimonial(testimonial, data, index)).join('')}
</div>`;
    }
    renderTestimonial(testimonial, data, index) {
        logger_1.logger.debug('TestimonialsRendererV3', 'renderTestimonial', 'Génération témoignage', {
...
<div class="testimonials__cta">
  ...
  <div class="testimonials__cta-buttons">
    ...
    generateCSS(data) {
        logger_1.logger.debug('TestimonialsRendererV3', 'generateCSS', 'Génération CSS spécifique');
        let css = '';
...
.testimonials .testimonial {
  --testimonials-radius: ${radiusMap[data.styles.borderRadius]};
    ...
    generateJS(data) {
        logger_1.logger.debug('TestimonialsRendererV3', 'generateJS', 'Génération JS', {
...
exports.TestimonialsRendererV3 = TestimonialsRendererV3;
