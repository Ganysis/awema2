...
exports.PricingRendererV3 = void 0;
const pricing_1 = require("../schemas/blocks/pricing");
const logger_1 = require("../core/logger");
class PricingRendererV3 {
    constructor() {
        this.type = 'pricing-ultra-modern';
        this.version = '3.0.0';
        logger_1.logger.info('PricingRendererV3', 'constructor', '🚀 Initialisation du renderer Pricing V3');
    }
    validate(data) {
        logger_1.logger.debug('PricingRendererV3', 'validate', 'Validation des données', {
            hasData: !!data,
            dataType: typeof data
        });
        const result = pricing_1.pricingDataSchema.safeParse(data);
        if (!result.success) {
            logger_1.logger.warn('PricingRendererV3', 'validate', 'Validation échouée', {
                errors: result.error.errors
            });
        }
        else {
            logger_1.logger.info('PricingRendererV3', 'validate', '✅ Validation réussie', {
    ...
    getDefaultData() {
        logger_1.logger.debug('PricingRendererV3', 'getDefaultData', 'Retour des données par défaut');
        return pricing_1.pricingDefaults;
    }
    getDefaultCSS() {
        logger_1.logger.debug('PricingRendererV3', 'getDefaultCSS', 'Génération CSS par défaut');
        return `
/* Pricing Base Styles */
.pricing {
  position: relative;
...
.pricing--padding-xl { padding: 6rem 0; }`;
    ...
    getRequiredAssets() {
        logger_1.logger.debug('PricingRendererV3', 'getRequiredAssets', 'Aucun asset requis');
        return [];
    }
    renderPreview(data) {
        logger_1.logger.debug('PricingRendererV3', 'renderPreview', 'Génération preview', {
...
<div class="pricing-preview">
  ...
  <p>${data.plans.length} plans - ${data.variant}</p>
    ...
    render(data, context) {
        ...
        logger_1.logger.info('PricingRendererV3', 'render', '🎨 Début du rendu Pricing', {
            variant: data.variant,
            plansCount: data.plans.length,
            hasContext: !!context
        });
        try {
            // Générer le HTML
            logger_1.logger.debug('PricingRendererV3', 'render', 'Génération HTML');
            const html = this.generateHTML(data);
            // Générer le CSS spécifique
            logger_1.logger.debug('PricingRendererV3', 'render', 'Génération CSS');
            const css = this.generateCSS(data);
            // Générer le JS si nécessaire
            logger_1.logger.debug('PricingRendererV3', 'render', 'Génération JS');
            const js = this.generateJS(data);
            // Calculer les performances
            const renderTime = performance.now() - startTime;
            logger_1.logger.info('PricingRendererV3', 'render', '✅ Rendu Pricing terminé', {
            ...
            logger_1.logger.error('PricingRendererV3', 'render', '❌ Erreur lors du rendu', error);
            return {
    ...
    generateHTML(data) {
        logger_1.logger.debug('PricingRendererV3', 'generateHTML', 'Génération structure HTML', {
            variant: data.variant,
            layout: data.layout
        });
        const classes = [
  ...
  <div class="pricing__container">
    ...
    renderHeader(data) {
        if (!data.title && !data.subtitle && !data.description)
            return '';
        logger_1.logger.debug('PricingRendererV3', 'renderHeader', 'Génération header');
        return `
<div class="pricing__header">
  ...
  ${data.description ? `<p class="pricing__description">${this.escape(data.description)}</p>` : ''}
    ...
    renderPeriodToggle(toggle) {
        logger_1.logger.debug('PricingRendererV3', 'renderPeriodToggle', 'Génération toggle', {
            style: toggle.style,
...
<div class="pricing__toggle pricing__toggle--${toggle.position}">
  ...
  ` : ''}
    ...
    renderPlans(data) {
        logger_1.logger.debug('PricingRendererV3', 'renderPlans', 'Génération plans', {
...
<div class="${gridClasses}">
  ${data.plans.map((plan, index) => this.renderPlan(plan, data, index)).join('')}
    ...
    renderPlan(plan, data, index) {
        logger_1.logger.debug('PricingRendererV3', 'renderPlan', 'Génération plan', {
            name: plan.name,
...
<article class="${classes}" style="${styles}" data-plan="${plan.id}">
  ...
  <div class="pricing__cta">
    ...
    renderPrice(price, data) {
        logger_1.logger.debug('PricingRendererV3', 'renderPrice', 'Génération prix', {
            amount: price.amount,
            period: price.period
        });
...
<div class="pricing__price">
  ...
  ` : ''}
    ...
    renderFeatures(features, data) {
        logger_1.logger.debug('PricingRendererV3', 'renderFeatures', 'Génération features', {
...
<a href="${cta.link}" class="${classes}">
  ${this.escape(cta.text)}
    ...
    renderComparisonTable(data) {
        logger_1.logger.debug('PricingRendererV3', 'renderComparisonTable', 'Génération tableau comparatif', {
...
<div class="pricing__comparison">
  <table class="pricing__table">
    ...
    renderGuarantees(guarantees) {
        logger_1.logger.debug('PricingRendererV3', 'renderGuarantees', 'Génération garanties', {
...
<div class="pricing__guarantees pricing__guarantees--${guarantees.style}">
  ...
  `).join('')}
    ...
    renderFAQ(faq) {
        logger_1.logger.debug('PricingRendererV3', 'renderFAQ', 'Génération FAQ', {
...
<div class="pricing__faq pricing__faq--${faq.position}">
  ...
  <div class="pricing__faq-items">
    ...
    renderCTA(cta) {
        logger_1.logger.debug('PricingRendererV3', 'renderCTA', 'Génération CTA', {
            style: cta.style,
...
<div class="pricing__cta-section pricing__cta-section--${cta.style}">
  ...
  <a href="${cta.buttonLink}" class="btn btn--${cta.buttonVariant} btn--lg">
    ...
    generateCSS(data) {
        logger_1.logger.debug('PricingRendererV3', 'generateCSS', 'Génération CSS spécifique');
        let css = '';
...
.pricing .pricing__plan--recommended {
  --recommended-scale: ${data.display.recommendedScale};
    ...
    generateJS(data) {
        logger_1.logger.debug('PricingRendererV3', 'generateJS', 'Génération JS', {
...
exports.PricingRendererV3 = PricingRendererV3;
