...
exports.FAQRendererV3 = void 0;
const faq_1 = require("../schemas/blocks/faq");
const logger_1 = require("../core/logger");
class FAQRendererV3 {
    constructor() {
        this.type = 'faq-ultra-modern';
        this.version = '3.0.0';
        logger_1.logger.info('FAQRendererV3', 'constructor', '🚀 Initialisation du renderer FAQ V3');
    }
    validate(data) {
        logger_1.logger.debug('FAQRendererV3', 'validate', 'Validation des données', {
            hasData: !!data,
            dataType: typeof data
        });
        const result = faq_1.faqDataSchema.safeParse(data);
        if (!result.success) {
            logger_1.logger.warn('FAQRendererV3', 'validate', 'Validation échouée', {
                errors: result.error.errors
            });
        }
        else {
            logger_1.logger.info('FAQRendererV3', 'validate', '✅ Validation réussie', {
    ...
    getDefaultData() {
        logger_1.logger.debug('FAQRendererV3', 'getDefaultData', 'Retour des données par défaut');
        return faq_1.faqDefaults;
    }
    getDefaultCSS() {
        logger_1.logger.debug('FAQRendererV3', 'getDefaultCSS', 'Génération CSS par défaut');
        return `
/* FAQ Base Styles */
.faq {
  position: relative;
...
.faq--radius-xl .faq__item { border-radius: 1rem; }`;
    ...
    getRequiredAssets() {
        logger_1.logger.debug('FAQRendererV3', 'getRequiredAssets', 'Aucun asset requis');
        return [];
    }
    renderPreview(data) {
        logger_1.logger.debug('FAQRendererV3', 'renderPreview', 'Génération preview', {
...
<div class="faq-preview">
  ...
  <p>${data.items.length} questions - ${data.variant}</p>
    ...
    render(data, context) {
        ...
        logger_1.logger.info('FAQRendererV3', 'render', '🎨 Début du rendu FAQ', {
            variant: data.variant,
            itemsCount: data.items.length,
        ...
        try {
            // Générer le HTML
            logger_1.logger.debug('FAQRendererV3', 'render', 'Génération HTML');
            const html = this.generateHTML(data);
            // Générer le CSS spécifique
            logger_1.logger.debug('FAQRendererV3', 'render', 'Génération CSS');
            const css = this.generateCSS(data);
            // Générer le JS
            logger_1.logger.debug('FAQRendererV3', 'render', 'Génération JS');
            const js = this.generateJS(data);
            // Calculer les performances
            const renderTime = performance.now() - startTime;
            logger_1.logger.info('FAQRendererV3', 'render', '✅ Rendu FAQ terminé', {
            ...
            logger_1.logger.error('FAQRendererV3', 'render', '❌ Erreur lors du rendu', error);
            return {
    ...
    generateHTML(data) {
        logger_1.logger.debug('FAQRendererV3', 'generateHTML', 'Génération structure HTML', {
            variant: data.variant,
            layout: data.layout
        });
        const classes = [
  ...
  <div class="faq__container">
    ...
    renderHeader(data) {
        if (!data.title && !data.subtitle && !data.description)
            return '';
        logger_1.logger.debug('FAQRendererV3', 'renderHeader', 'Génération header');
        return `
<div class="faq__header faq__header--${data.layout.alignment}">
  ...
  ${data.description ? `<p class="faq__description">${this.escape(data.description)}</p>` : ''}
    ...
    renderSearch(data) {
        logger_1.logger.debug('FAQRendererV3', 'renderSearch', 'Génération recherche', {
...
<div class="faq__search faq__search--${data.search.position}">
  ...
  <div class="faq__search-results" style="display: none;">
    ...
    renderCategories(data) {
        logger_1.logger.debug('FAQRendererV3', 'renderCategories', 'Génération catégories', {
...
<div class="faq__categories faq__categories--${data.categories.style}">
  ...
  `).join('')}
    ...
    renderFAQList(data) {
        logger_1.logger.debug('FAQRendererV3', 'renderFAQList', 'Génération liste FAQ', {
            itemsCount: data.items.length,
...
<div class="faq__cta faq__cta--${cta.style}">
  ...
  <a href="${cta.buttonLink}" class="btn btn--${cta.buttonVariant} btn--lg">
    ...
    generateCSS(data) {
        logger_1.logger.debug('FAQRendererV3', 'generateCSS', 'Génération CSS spécifique');
        let css = '';
...
.faq {
  ...
  --faq-answer-height: ${data.display.answerStyle.lineHeight === 'tight' ? '1.25' :
    ...
    generateJS(data) {
        logger_1.logger.debug('FAQRendererV3', 'generateJS', 'Génération JS', {
            hasSearch: data.search.enabled,
            hasCategories: data.categories.enabled,
            hasFeedback: data.feedback.enabled,
            expandBehavior: data.display.expandBehavior
...
exports.FAQRendererV3 = FAQRendererV3;
