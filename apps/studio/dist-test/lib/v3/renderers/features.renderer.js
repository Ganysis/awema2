...
exports.FeaturesRendererV3 = void 0;
const features_1 = require("../schemas/blocks/features");
const logger_1 = require("../core/logger");
class FeaturesRendererV3 {
    constructor() {
        this.type = 'features-ultra-modern';
        this.version = '3.0.0';
        logger_1.logger.info('FeaturesRendererV3', 'constructor', '🚀 Initialisation du renderer Features V3');
    }
    validate(data) {
        logger_1.logger.debug('FeaturesRendererV3', 'validate', 'Validation des données', {
            hasData: !!data,
            dataType: typeof data
        });
        const result = features_1.featuresDataSchema.safeParse(data);
        if (!result.success) {
            logger_1.logger.warn('FeaturesRendererV3', 'validate', 'Validation échouée', {
                errors: result.error.errors
            });
        }
        else {
            logger_1.logger.info('FeaturesRendererV3', 'validate', '✅ Validation réussie', {
    ...
    getDefaultData() {
        logger_1.logger.debug('FeaturesRendererV3', 'getDefaultData', 'Retour des données par défaut');
        return features_1.featuresDefaults;
    }
    getDefaultCSS() {
        logger_1.logger.debug('FeaturesRendererV3', 'getDefaultCSS', 'Génération CSS par défaut');
        return `
...
.features--padding-xl { padding: 6rem 0; }`;
    ...
    getRequiredAssets() {
        logger_1.logger.debug('FeaturesRendererV3', 'getRequiredAssets', 'Aucun asset requis');
        return [];
    }
    renderPreview(data) {
        logger_1.logger.debug('FeaturesRendererV3', 'renderPreview', 'Génération preview', {
...
<div class="features-preview">
  ...
  <p>${data.features.length} features - ${data.variant}</p>
    ...
    render(data, context) {
        ...
        logger_1.logger.info('FeaturesRendererV3', 'render', '🎨 Début du rendu Features', {
            variant: data.variant,
            featuresCount: data.features.length,
            hasContext: !!context
        });
        try {
            // Générer le HTML
            logger_1.logger.debug('FeaturesRendererV3', 'render', 'Génération HTML');
            const html = this.generateHTML(data);
            // Générer le CSS spécifique
            logger_1.logger.debug('FeaturesRendererV3', 'render', 'Génération CSS');
            const css = this.generateCSS(data);
            // Générer le JS si nécessaire
            logger_1.logger.debug('FeaturesRendererV3', 'render', 'Génération JS');
            const js = this.generateJS(data);
            // Calculer les performances
            const renderTime = performance.now() - startTime;
            logger_1.logger.info('FeaturesRendererV3', 'render', '✅ Rendu Features terminé', {
            ...
            logger_1.logger.error('FeaturesRendererV3', 'render', '❌ Erreur lors du rendu', error);
            return {
    ...
    generateHTML(data) {
        logger_1.logger.debug('FeaturesRendererV3', 'generateHTML', 'Génération structure HTML', {
            variant: data.variant,
            layout: data.layout
        });
        const classes = [
  ...
  <div class="features__container">
    ...
    renderHeader(data) {
        if (!data.title && !data.subtitle && !data.description)
            return '';
        logger_1.logger.debug('FeaturesRendererV3', 'renderHeader', 'Génération header');
        return `
<div class="features__header features__header--${data.layout.alignment}">
  ...
  ${data.description ? `<p class="features__description">${this.escape(data.description)}</p>` : ''}
    ...
    renderFilters(data) {
        logger_1.logger.debug('FeaturesRendererV3', 'renderFilters', 'Génération filtres', {
...
<div class="features__filters features__filters--${data.filtering.style}">
  ...
  `).join('')}
    ...
    renderFeatures(features, data) {
        logger_1.logger.debug('FeaturesRendererV3', 'renderFeatures', 'Génération features', {
    ...
    renderGridFeatures(features, data) {
...
<div class="${gridClasses}">
  ${features.map((feature, index) => this.renderFeature(feature, data, index)).join('')}
    ...
    renderTimelineFeatures(features, data) {
        return `
<div class="features__grid features__timeline">
  ${features.map((feature, index) => this.renderFeature(feature, data, index)).join('')}
</div>`;
    }
    renderCarouselFeatures(features, data) {
        return `
<div class="features__carousel-wrapper">
  ...
  ` : ''}
    ...
    renderTabsFeatures(features, data) {
...
<div class="features__tabs-wrapper">
  ...
  <div class="features__tabs-content">
    ...
    renderFeature(feature, data, index) {
...
<article class="${classes}" style="${styles}" data-category="${feature.category || 'general'}">
  ...
  <div class="feature__content">
    ...
    renderFeatureIcon(feature, data) {
...
<div class="features__cta">
  <a href="${cta.link}" class="btn btn--${cta.variant} btn--lg">
    ...
    generateCSS(data) {
        logger_1.logger.debug('FeaturesRendererV3', 'generateCSS', 'Génération CSS spécifique');
        let css = '';
...
.features .feature {
  border-radius: ${radiusMap[data.styles.borderRadius]};
    ...
    generateJS(data) {
        logger_1.logger.debug('FeaturesRendererV3', 'generateJS', 'Génération JS', {
...
exports.FeaturesRendererV3 = FeaturesRendererV3;
