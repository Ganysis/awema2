...
exports.v2ToV3Adapter = exports.V2ToV3Adapter = void 0;
const logger_1 = require("../core/logger");
class V2ToV3Adapter {
    constructor() {
        this.conversionMap = new Map();
        this.transformers = new Map();
        logger_1.logger.info('V2ToV3Adapter', 'constructor', '🔄 Initialisation de l\'adaptateur V2→V3');
    ...
    initializeConversionMap() {
        logger_1.logger.debug('V2ToV3Adapter', 'initializeConversionMap', '📋 Initialisation de la map de conversion');
        ...
        logger_1.logger.info('V2ToV3Adapter', 'initializeConversionMap', `✅ ${this.conversionMap.size} types mappés`);
    }
    /**
     * Transformateurs spécifiques par type
     */
    initializeTransformers() {
        logger_1.logger.debug('V2ToV3Adapter', 'initializeTransformers', '🔧 Initialisation des transformateurs');
        // Hero transformer
        this.transformers.set('hero-ultra-modern', (v2) => {
            logger_1.logger.debug('V2ToV3Adapter', 'transformHero', 'Transformation Hero V2→V3', { v2 });
            return {
        ...
        // Contact transformer
        this.transformers.set('contact-ultra-modern', (v2) => {
            logger_1.logger.debug('V2ToV3Adapter', 'transformContact', 'Transformation Contact V2→V3', { v2 });
            return {
                variant: v2.variant || 'gradient-wave',
        ...
        // Header transformer
        this.transformers.set('header-ultra-modern', (v2) => {
            logger_1.logger.debug('V2ToV3Adapter', 'transformHeader', 'Transformation Header V2→V3', { v2 });
            return {
        ...
        // Footer transformer
        this.transformers.set('footer-ultra-modern', (v2) => {
            logger_1.logger.debug('V2ToV3Adapter', 'transformFooter', 'Transformation Footer V2→V3', { v2 });
            return {
        ...
        logger_1.logger.info('V2ToV3Adapter', 'initializeTransformers', `✅ ${this.transformers.size} transformateurs créés`);
    }
    /**
     * Convertit un bloc V2 en BlockData V3
     */
    convertBlock(v2Block) {
        const startTime = performance.now();
        logger_1.logger.info('V2ToV3Adapter', 'convertBlock', `🔄 Conversion bloc V2→V3: ${v2Block.type}`, {
            id: v2Block.id,
            type: v2Block.type,
        ...
        try {
            ...
            if (!v3Type) {
                logger_1.logger.warn('V2ToV3Adapter', 'convertBlock', `⚠️ Type non mappé: ${v2Block.type}`);
                return null;
            }
            ...
            let v3Data;
            if (transformer) {
                logger_1.logger.debug('V2ToV3Adapter', 'convertBlock', `🔧 Application du transformateur: ${v3Type}`);
                v3Data = transformer(v2Data);
            }
            else {
                logger_1.logger.warn('V2ToV3Adapter', 'convertBlock', `⚠️ Pas de transformateur pour: ${v3Type}, copie directe`);
                v3Data = { ...v2Data };
            }
            ...
            logger_1.logger.info('V2ToV3Adapter', 'convertBlock', `✅ Bloc converti: ${v2Block.type} → ${v3Type}`, {
                duration: performance.now() - startTime,
                dataKeys: Object.keys(v3Data)
            });
            logger_1.logger.performance('V2ToV3Adapter', 'convertBlock', startTime);
            return blockData;
        }
        catch (error) {
            logger_1.logger.error('V2ToV3Adapter', 'convertBlock', `❌ Erreur lors de la conversion: ${v2Block.type}`, error);
            return null;
        }
    }
    /**
     ...
     */
    convertProject(v2Project) {
        const startTime = performance.now();
        logger_1.logger.info('V2ToV3Adapter', 'convertProject', '📦 Conversion projet V2→V3', {
        ...
        try {
            ...
            if (v2Project.globalHeader) {
                logger_1.logger.debug('V2ToV3Adapter', 'convertProject', '📌 Conversion du header global');
                const header = this.convertBlock(v2Project.globalHeader);
                if (header)
                    blocks.push(header);
                else
                    errors.push({ type: 'header', error: 'Conversion échouée' });
            }
            // 2. Convertir les blocs de la page principale
            const mainPage = v2Project.pages?.find(p => p.slug === '/') || v2Project.pages?.[0];
            if (mainPage?.blocks) {
                logger_1.logger.debug('V2ToV3Adapter', 'convertProject', `📄 Conversion de ${mainPage.blocks.length} blocs de la page principale`);
                mainPage.blocks.forEach((block, index) => {
                    const converted = this.convertBlock(block);
                    if (converted) {
                        blocks.push(converted);
                        logger_1.logger.debug('V2ToV3Adapter', 'convertProject', `✅ Bloc ${index + 1}/${mainPage.blocks.length} converti`);
                    }
                    else {
                        errors.push({ type: block.type, index, error: 'Conversion échouée' });
                        logger_1.logger.warn('V2ToV3Adapter', 'convertProject', `⚠️ Bloc ${index + 1} non converti: ${block.type}`);
                    }
                });
            }
            // 3. Convertir le footer global
            if (v2Project.globalFooter) {
                logger_1.logger.debug('V2ToV3Adapter', 'convertProject', '📌 Conversion du footer global');
                const footer = this.convertBlock(v2Project.globalFooter);
                if (footer)
                    blocks.push(footer);
                else
                    errors.push({ type: 'footer', error: 'Conversion échouée' });
            }
            ...
            logger_1.logger.info('V2ToV3Adapter', 'convertProject', `✅ Projet converti avec succès`, {
                blocksConverted: blocks.length,
                errorsCount: errors.length,
                duration: performance.now() - startTime
            });
            if (errors.length > 0) {
                logger_1.logger.warn('V2ToV3Adapter', 'convertProject', `⚠️ ${errors.length} erreurs lors de la conversion`, errors);
            }
            logger_1.logger.performance('V2ToV3Adapter', 'convertProject', startTime);
            return { blocks, metadata };
        }
        catch (error) {
            logger_1.logger.error('V2ToV3Adapter', 'convertProject', '❌ Erreur critique lors de la conversion du projet', error);
            throw error;
        }
    }
    /**
     ...
     */
    transformButton(v2Button) {
        if (!v2Button)
            return null;
        logger_1.logger.debug('V2ToV3Adapter', 'transformButton', 'Transformation bouton', v2Button);
        return {
    ...
    transformContactForm(v2Form) {
        logger_1.logger.debug('V2ToV3Adapter', 'transformContactForm', 'Transformation formulaire', v2Form);
    ...
    transformFooterColumns(v2Columns) {
...
exports.V2ToV3Adapter = V2ToV3Adapter;
// Export singleton instance
exports.v2ToV3Adapter = V2ToV3Adapter.getInstance();
