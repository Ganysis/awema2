-- AWEMA CMS Multi-tenant Schema - Ultra Pro Edition
-- Version: 2.0
...
-- Partitions mensuelles pour les analytics
...
CREATE POLICY super_admin_all ON sites
...
-- Politique pour les clients (accès à leur site uniquement)
CREATE POLICY client_own_site ON sites
    FOR ALL TO authenticated
    USING (
        id IN (
            SELECT site_id FROM cms_users 
            WHERE id = auth.uid()
        )
    );

-- Politique pour le contenu (accès selon le site)
CREATE POLICY content_site_access ON content
    FOR ALL TO authenticated
    USING (
        site_id IN (
            SELECT site_id FROM cms_users 
...
CREATE POLICY media_site_access ON media
    FOR ALL TO authenticated
    USING (
        site_id IN (
            SELECT site_id FROM cms_users 
...
-- Note: L'utilisateur admin sera créé via Supabase Auth
-- Pour créer un super admin, utiliser la console Supabase ou l'API

-- =====================================================
-- JOBS PLANIFIÉS (à configurer dans Supabase)
-- =====================================================

-- 1. Nettoyer le cache expiré (toutes les heures)
-- SELECT cleanup_expired_cache();

-- 2. Calculer les statistiques (toutes les nuits)
-- REFRESH MATERIALIZED VIEW site_stats;

-- 3. Archiver les analytics anciennes (tous les mois)
-- CREATE TABLE analytics_events_archive AS 
-- SELECT * FROM analytics_events WHERE created_at < NOW() - INTERVAL '3 months';
...
GRANT SELECT ON content, media TO anon;
GRANT INSERT ON form_submissions, analytics_events TO anon;
...
-- Créer des statistiques étendues pour les jointures fréquentes
CREATE STATISTICS site_content_stats (dependencies) ON site_id, page_id FROM content;
CREATE STATISTICS site_media_stats (dependencies) ON site_id, folder FROM media;