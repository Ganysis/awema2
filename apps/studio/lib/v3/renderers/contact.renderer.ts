...
import { contactDataSchema, contactDefaults, type ContactData } from '../schemas/blocks/contact';

export class ContactRendererV3 implements BlockRenderer<ContactData> {
...
<div class="contact-preview">
  ...
  private generateHTML(data: ContactData): string {
    const { 
      variant, 
      layout,
      title, 
...
<section class="${classes}" id="contact" data-block="contact">
  ...
  private renderHeader(title: string, subtitle?: string, description?: string): string {
    return `
<div class="contact__header">
  ...
  private renderFormSection(form: ContactData['form'], variant: string): string {
    return `
...
<div class="form-group">
  ...
  private renderInfoSection(info: ContactData['contactInfo'], map?: ContactData['map']): string {
    return `
<div class="contact__info-section">
  ...
  private renderContactInfo(info: ContactData['contactInfo']): string {
    return `
<div class="contact__info">
  ...
  private renderMap(map: NonNullable<ContactData['map']>): string {
...
<div class="contact__map">
  ...
  private generateCSS(data: ContactData): string {
    let css = '';
...
.contact--floating-cards .contact__form:hover {
  ...
  private generateJS(data: ContactData): string {
    const { form, map } = data;
