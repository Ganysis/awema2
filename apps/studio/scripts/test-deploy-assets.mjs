...
async function testDeploymentAssets() {
  ...
  // Import des services (dynamique pour TypeScript)
  const { StaticExportService } = await import('../lib/services/static-export-simplified.js');
  ...
  try {
    ...
    const testDir = join(__dirname, 'test-deploy-output');
    await fs.mkdir(testDir, { recursive: true });
    await fs.mkdir(join(testDir, 'assets/css'), { recursive: true });
    await fs.mkdir(join(testDir, 'assets/js'), { recursive: true });
    
    // Sauvegarder les fichiers
    await fs.writeFile(join(testDir, 'index.html'), exportData.html);
    await fs.writeFile(join(testDir, 'assets/css/styles.css'), exportData.css);
    await fs.writeFile(join(testDir, 'assets/js/main.js'), exportData.js);
    
    // Sauvegarder les assets
    for (const asset of exportData.assets) {
      const assetPath = join(testDir, asset.path);
